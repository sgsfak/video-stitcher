...
def test_fns():
    l = ["video-20200519-%02d%02d00.mp4" % (x,y) 
    for x in range(0, 23)
...
def fn_to_dt(fn):
    # datetime.datetime.strptime('video-20200519-223001.mp4', "video-%Y%m%d-%H%M%S.mp4")
    dt = datetime.datetime.strptime(fn, "video-%Y%m%d-%H%M%S.mp4")
    return dt.timestamp()
...
def locate(ts, files, period_mins=5):
    ...
    print("centering at %s, start=%s, si=%d, end=%s sj=%d" % (
        datetime.datetime.fromtimestamp(ts),
        datetime.datetime.fromtimestamp(start), si, datetime.datetime.fromtimestamp(end), sj))
    ret = []
...
def stitch(lst):
    ...
    fd,concatsf = tempfile.mkstemp(".txt", prefix="stitcher-conc-", dir="/tmp")
    ...
    concat_cmd = "ffmpeg -y -safe 0 -f concat -i %s -codec copy %s" % (concatsf, output)
